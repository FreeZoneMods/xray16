...
void game_cl_RolePlay::OnTeamMenuBack() //------------------------------------------------ÍÀÇÀÄ ÈÇ ÂÛÁÎÐÀ ÊÎÌÀÍÄÛ---------ÑÊÐÛÂÀÅÒÑß ÎÊÍÎ È ÏÐÎÈÑÕÎÄÈÒ ÑËÅÄÓÞÙÅÅ: 
{
...
void game_cl_RolePlay::OnSkinMenuBack() //--------------------------------------------------ÍÀÇÀÄ ÈÇ ÑÊÈÍÎÂ---------ÑÊÐÛÂÀÅÒÑß ÎÊÍÎ È ÏÐÎÈÑÕÎÄÈÒ ÑËÅÄÓÞÙÅÅ: 
{
	//if (CanCallTeamSelectMenu())
	//	m_game_ui->m_pUITeamSelectWnd->ShowDialog(true);
	////.		m_game_ui->StartStopMenu(m_game_ui->m_pUITeamSelectWnd, true);
...
bool game_cl_RolePlay::IsEnemy(game_PlayerState* ps) //---------m4d_? Êòî âðàã
{
	if (!local_player) return false;
...
void game_cl_RolePlay::UpdateMapLocations()
{
	...
	if (local_player && !Level().CurrentEntity()) //-----m4d_pda (fix: Íà êàðòå self ïîêàçûâàëñÿ êàê friend)
	{
		PLAYERS_MAP_IT it = players.begin();
		for (; it != players.end(); ++it)
		{
