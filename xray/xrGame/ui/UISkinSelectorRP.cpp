...
CUISkinSelectorWndRP::CUISkinSelectorWndRP(const char* strSectionName, s16 team)
{	
	m_team			= team;
	m_iActiveIndex	= -1;
...
CUISkinSelectorWndRP::~CUISkinSelectorWndRP()
{
...
void CUISkinSelectorWndRP::InitSkins()
{
...
void CUISkinSelectorWndRP::UpdateSkins()
{
	for (int i = 0; i < 3; i++)
	{
		if (!!m_shader)
...
void CUISkinSelectorWndRP::Init(const char* strSectionName)
{
	R_ASSERT(0 != strSectionName[0]);
	m_strSection = strSectionName;
	
	CUIXml xml_doc;
	xml_doc.Load(CONFIG_PATH, UI_PATH, "skin_selector_rp.xml");
...
void CUISkinSelectorWndRP::SendMessage(CUIWindow *pWnd, s16 msg, void *pData)
{
...
void CUISkinSelectorWndRP::OnBtnCancel()
{
...
void CUISkinSelectorWndRP::OnBtnOK()
{
	HideDialog();
...
bool CUISkinSelectorWndRP::OnMouseAction(float x, float y, EUIMessages mouse_action)
{
	return CUIWindow::OnMouseAction(x,y,mouse_action);	
}

bool CUISkinSelectorWndRP::OnKeyboardAction(int dik, EUIMessages keyboard_action)
{
...
void CUISkinSelectorWndRP::OnKeyLeft()
{	
	if (m_firstSkin > 0)
	{
		m_firstSkin--;
...
void CUISkinSelectorWndRP::OnKeyRight()
{
...
int	CUISkinSelectorWndRP::GetActiveIndex(){
	if (-1 == m_iActiveIndex)
		return -1;
	else
...
void CUISkinSelectorWndRP::SetVisibleForBtn(ESKINMENU_BTN btn, bool state){
	switch (btn)
	{
...
void CUISkinSelectorWndRP::SetCurSkin(int skin){
...
void CUISkinSelectorWndRP::Update			()
{
	UpdateSkins();
	inherited::Update();
}